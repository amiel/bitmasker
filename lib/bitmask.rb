...
class Bitmask
  ...
  def to_i; @data end
  def to_a; @masks.keys.sort{|a,b| @masks[a] <=> @masks[b] }.collect{|k| [k, get(k)]} end
  def to_h; @masks.keys.inject({}){|h,k| h[k] = get k; h} end

  # def [](attr); get attr end
  # def []=(attr, value); set attr, value end
